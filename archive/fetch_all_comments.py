...
def fetch_all_comments(bv_id):
    """获取所有评论用户"""
    print(f"\n[API获取] 获取视频 {bv_id} 的所有评论用户...")
    ...
    try:
        ...
        while True:
            ...
            if comment_data.get('code') != 0:
                print(f"[错误] 获取评论失败: {comment_data.get('message')}")
                break
            ...
            print(f"  [进度] 本页获取 {len(replies)} 条评论，累计 {len(users)} 位用户")
...
def main():
    ...
    users = fetch_all_comments(bv_id)
